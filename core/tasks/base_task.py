...
from celery import states #noqa

import pprint

from core.utils import DICT_CREDENTIALS, DICT_CREDENTIALS_TYPE
...
from core.utils.sshkeys_utils import OPENSSH_KEY_ERROR
from core.utils.common import get_ssh_version
from core.helpers import commit_alive_event
...
from celery.utils.log import get_task_logger as get_celery_task_logger
...
# logger = get_celery_task_logger("celerytask")
...
class BaseTask(Task):
    ...
    def run(self, instance, **kwargs):
        ...
        # Rewrite logic behind instance
        self.instance = self.build__instance( instance.get('fields'), model=self.model )

        self.namespace = self.get__namespace( self.instance )

        self.event_ct = 0
        self.event_total = 100

        self.action = kwargs.get("action", self.instance.get('action'))
        logger.info("TASK Started. Action: '{}'".format(self.action))

        #progress bar
        
        # self.set__progress()
        verb = "starting"
        if self.action in ['stop']:
            verb = "stopping"
        if self.action in ['shutdown']:
            verb = "shuting down"
        if self.action in ['check']:
            verb = "checking"

        self.task_id = kwargs.get("task_id") or uuid()
        ...
        self.emit__channel_notification("start")
        ...
        private_data_dir = self.build__private_data_dir(self.instance)

        sshkey = self.build__sshkey(self.instance, private_data_dir)
        ...
        logger.info("*** TAKS KWARGS: {}".format(pformat(kwargs, indent=4)))

        
        extravars =  self.build__extravars( self.instance, private_data_dir, dict() )
        
        # logger.info(extravars)

        envvars =  self.build__envvars(self.instance, private_data_dir, **opts)

        inventory = self.build__inventory(self.instance, private_data_dir, **opts)


        self.roles_path = self.build__roles(self.instance, private_data_dir, self.namespace)

        process_isolation_params = self.build__params_process_isolation(self.instance, private_data_dir, 'cwd')
        ...
        # print("**** ANSIBLE RUN ARGUMENTS ****")
        logger.info("Final runner_args : \n{}".format(runner_args))
        # pprint(runner_args)
    ...
    def build__instance(self, instance, **kwargs):
        """
        set final instance to process
        """
        ...
        logger.info(f"****** KWARGS MODEL : { model }")
        logger.info(f"****** APPLICATIONS : {instance.get('applications')}")


        logger.info(f"******INITIAL INPUTS : \n{instance.get('inputs')}")

        # INVENTORY
        if kwargs.get('model') not in ['core.usercredential']:
            extra_creds = {'usercredential': instance.get('usercredential')}
            parent = self.get__parent_credentials(instance, extra_creds)
            inventory = dict()
            ...
            instance['inputs'].update( dict( 
                    inventory    = inventory.get('ansible_host', 'localhost'),
                    ansible_host = inventory.get('ansible_host', 'localhost'), 
                    username     = inventory.get('ansible_user', 'root'),
                    ansible_user = inventory.get('ansible_user', 'root'),
                    )
                )

        logger.info(f"****** FINAL INVENTORY INPUTS : \n{instance.get('inputs')}")

        # SSH KEYS

        # jobs connect 
        #     - cloud based:  no ssh key required
        #     - non-cloud based: ssh key user credentials 

        # job deployment
        #     -  cloud based: iaas pub key, 
        #     - non-cloud: usercredentials pub key / usercredential private ssh key to connect.
        # virtual machine job:
        #     - cloud based: iaas private key 
        # non cloud based: iaas private key 


        # PROVIDER CONNECT - CLOUD
        # ----- no ssh required
        if model in ['core.usercredential']:
            pass

        # PROVIDER CONNECT - NON CLOUD
        # ----- usercredential sshkey
        if model in ['core.usercredential']:
            pass

        # DEPLOYMENT JOB - CLOUD
        # ----- iaas pub key / iaas private key
        if model in ['core.iaas'] and cloud:
            pass


        # DEPLOYMENT JOB - NON CLOUD
        # ----- usercredential sshkey to connect / iaas public key for authorized_keys
        if model in ['core.iaas'] and not cloud:
            pass
            usercredential_inputs = self.get__instance_inputs(instance, 'core.usercredential')
            privatekey = usercredential_inputs.get('inputs').get('privatekey')
            publickey  = usercredential_inputs.get('inputs').get('publickey')
            if privatekey:
                instance['inputs'].update( dict( 
        ...
        # VIRTUAL MACHINE / APPLICATION - CLOUD/NON CLOUD
        # ----- iaas sshkey
        if model in ['core.virtualmachine']:
            pass
            iaas_inputs = self.get__instance_inputs(instance, 'core.iaas')
            privatekey = iaas_inputs.get('inputs').get('privatekey')
            publickey  = iaas_inputs.get('inputs').get('publickey')
        ...
        # VIRTUAL MACHINE - NON CLOUD
        # ----- iaas sshkey

        # if kind not in ['generic', 'host'] and not cloud and _model not in ['core.usercredential'] :
        #     # get user credential instead
        #     usercredential_inputs = self.get__instance_inputs(instance, 'core.usercredential')

        #     # logger.info(f"****** SSHKEY INPUTS : \n{usercredential_inputs}")
        #     privatekey = usercredential_inputs.get('inputs').get('privatekey')
        #     publickey  = usercredential_inputs.get('inputs').get('publickey')
        ...
        #             )
        #         )
        #     #KVM based instance - only ssh pub key from iaas

        # if kind in ['generic', 'host'] or not instance.get('applications'):
        #     # get user credential instead
        #     iaas_inputs = self.get__instance_inputs(instance, 'core.iaas')
        ...
        # if kind not in ['generic', 'host'] and instance.get('applications'):
        #     # get user credential instead
        #     iaas_inputs = self.get__instance_inputs(instance, 'core.iaas')
        ...
        logger.info(f"******FINAL SSH_KEYS INPUTS : \n{instance.get('inputs')}")
        ...
        if model in ['core.iaas'] and cloud:
            pass
            usercredential_inputs = self.get__instance_inputs(instance, 'core.usercredential').get('inputs')
            usercredential_inputs.pop('privatekey', None)
            usercredential_inputs.pop('publickey', None)

            instance['inputs'].update( usercredential_inputs )

        # if kwargs.get('model') not in ['core.usercredential']:
        #     extra_creds = {'usercredential': instance.get('usercredential')}
        #     parent = self.get__parent_credentials(instance, extra_creds)

        #     # logger.info(f"******ENV VARS PARENT INPUTS : \n{parent.get('inputs')}")

        #     if cloud or parent.get('cloud'):

        #         if parent.get('inputs'):
        #             instance['inputs'].update( 
        #                 parent.get( 'inputs', dict() )
        #                 )

        logger.info(f"****** FINAL INPUTS : {instance.get('inputs')}")
    ...
    def set__progress(self, counter=0):
        if self.event_ct < self.event_total:
            self.update_state(state='PROGRESS', meta={'current': self.event_ct, 'total': self.event_total})
            self.event_ct += 10
    ...
    def get__parent_credentials(self, instance, extra_creds=None):
        """ Load Credentials into parameters """

        parent = dict()
        extra_creds = extra_creds or {}
        inherit_credentials = self.helper__merge_dicts(extra_creds, instance.get('credentials'))
    ...
    def build__envvars(self, instance, private_data_dir, **kwargs):
        ...
        inputs = {k.upper():v for k,v in inputs.items()}
        ...
        inputs.update(dict(
            ...
            # MS AZURE
            REQUESTS_CA_BUNDLE = f"{venv_path}/lib/python3.7/site-packages/certifi/cacert.pem",
        ...
        #Remove privatekey from env
        for k in ['privatekey', 'PRIVATEKEY']:        
            if inputs.get(k):
                inputs.pop(k, None)
    ...
    def helper__create_yaml(self, private_data_dir, directory, file, content, header = None):
        ...
        logger.debug("Yaml file '{}' created".format(path))
        return path
    ...
    def build__sshkey(self, instance, private_data_dir):
        from core.utils.sshkeys_utils import sshkey_sotre_to_filesystem
        return sshkey_sotre_to_filesystem(instance, private_data_dir)
        ...
        if private_data is not None:
            ...
            for credential, data in private_data.get('credentials', {}).items():
                ...
                # env variable.
                    ...
                    logger.debug("Private file '{}' created".format(path))
            ...
            for credential, data in private_data.get('certificates', {}).items():
                ...
                logger.debug("Private data file '{}' created".format(path))
                os.chmod(path, stat.S_IRUSR | stat.S_IWUSR)
...
# TODO: Move credential to proper secured model
    ...
    def build__inventory(self, instance, private_data_dir, **kwargs):
        ...
        inputs = instance.get('inputs')
        cloud = instance.get('cloud', False)
        ...
        if inventory.get("ansible_user") not in ['root'] and not cloud:
            inventory.update(dict(
                ansible_become = True,
                ansible_become_method = 'sudo', # enable for network gears
                )
            )
    ...
    def build__roles(self, instance, private_data_dir, namespace):
        ...
        source = '{}/{}'.format(settings.AHOME_LOCAL_ANSIBLE_ROLES_PATH, namespace)
        destination = '{}/{}'.format(ROLES_FOLDER, namespace)

        logger.info("role path:  {}".format(source))
# TODO
        if os.path.exists(source):
            copy_tree( source, os.path.join(private_data_dir, destination) )
            return os.path.join(private_data_dir, 'roles')
        else:
            logger.warning( "Source path Does Not Exist: {}".format(source) )

    def save__job(self):
        ident = self.instance.get('uuid')
        model = self.get__model_obj()

        logger.debug("Updating '{}': by uuid=[{}]...".format(model, ident))

        obj = model.objects.filter(uuid=ident).first()
        obj.status = self.job_defaults.get("status")
        obj.action = self.action
        obj.runner = self.job_defaults
        obj.save()
    ...
    def event_handler(self, event_data):
        ...
        if progress <= 90 :
            self.job_defaults.update(dict(celery_progress=progress))
        ...
        if event_data.get('event') == 'playbook_on_stats':
            ...
            self.job_defaults.update(dict(stats=stats))
    ...
    def finished_callback(self, runner_obj):
        logger.debug("FINISH TASK. STATUS: {} Runner: {}".format(runner_obj.rc, runner_obj))
        # pass
        self.job_defaults.update(dict(
        ...
        self.save__job()
        self.notifications.update(dict(alert=runner_obj.status, verb=runner_obj.status))
        self.emit__channel_notification("finish")
    ...
    def status_handler(self, status_data, runner_config):
        self.job_defaults.update(dict(status=status_data['status']))
        save_job = self.save__job()
            ...
            for k in e_data.keys():
                ...
                if len(query_output) > 0:
                    ...
                    for query_data in query_output:
                        ...
                        event_notify = query_data.get('metadata').get('notify',[])
                            ...
                            if len(query_entries) > 0:
                                for entry_data in query_entries:
                                    ...
                                    if data:
                                        ...
                                        if event_notify:
                                            for e_notify in event_notify:
                                                if e_notify not in ['', '*']:
                                                    self.notifications.update(dict( alert="info", verb=f"{e_notify}: {data.get('setfacts').get(e_notify)}" ))
                                                    self.emit__channel_notification("finish")
...
## - TODO: Review Orphan Clean up
# idea create a sub task to periodically check IP... if fail after 3/10 requests... Then clean up
            ...
            for qs in queryset:
                ...
                for unique_key in unique_keys:
                    ...
                    if qs.get('unique_keys') == unique_key:
                        ...
                        continue
                # REVIEW THE DELETE PROCESS
                if remove > 0:
                    # obj.objects.filter( id = remove ).delete()
                    obj.objects.filter( id = remove ).update(sn = 'ORPHAN')
    ...
    def workflow__job_save(self, instance, query_data, data):
        ...
        model, objname = self.get__model(query_data.get('metadata').get('model'))
        fobj = model.objects.filter(unique_keys__contains = data.get('unique_keys'))

        if fobj:
            for ds in fobj:
                qobj = model.objects.get( pk=ds.id )
            ...
            logger.debug("CREATE Object: {} Data: {}".format(model, pprint.pformat(data, indent=4)))
            # print("CREATE 1 ***")
    ...
    def collect__ansible_facts(self, facts):
        ...
        #logger.debug("COLLECTED FACTS: {}".format(pformat(output, indent=4)))
        return output
    ...
    def save__ansible_facts(self, instance, event_data, namespace):
        ...
        if event_data.get('task_action') in ['setup', 'gather_facts'] :
            ...
            if model_name in DEVICE_TEMPLATES:
                _obj, _ignore = model.objects.update_or_create(
                    uuid=instance.get('uuid'),
                    defaults=defaults)
        ...
        #logger.debug("ANSIBLE FACTS: {}".format(pformat(facts, indent=4)))
        return facts
    ...
    @property
    def uuname(self):
        return self.roles_path[len(settings.AHOME_PROOT_BASE_PATH)+1:-len('/roles')] if self.roles_path else ""

    def emit__channel_notification(self, life_cycle):
        """
        notification = dict(
                group
                type
        ...
        logger.info("TASK NOTIFICATION {}".format(pprint.pformat(args, indent=4)))
        commit_alive_event(args.get('verb'), self.model, self.pk, self.task_id, life_cycle)
        return args
