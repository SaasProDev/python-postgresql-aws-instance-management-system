...
from frontend.forms import ProjectForm as ahomeForm
...
"""
JUST (an updated) COPY FROM ORGANISATION
"""
...
def projects(request):
    _logger.info("Frontend Api : '{}'".format(request))
...
def project_create(request):
    ...
    query_set = user.get_available_objects(Organization, mode='execute')
...
def project_delete(request, pk):
    instance = get_object_or_404(MODEL_OBJ, pk=pk)
    data = dict({"status": "NOT IMPLEMENTED"})
