...
from frontend.forms import PaaSForm as ahomeForm
...
URL_CREATE = 'paas_wizardbox_create' #'%s_create' % MODEL_NAME.lower()
...
URL_DETAIL = 'paas_detail'
PAAS_PLATFORMS = 'paas_platforms'
...
def paas(request):
    ...
    # todo caching apply
    for paas_data in CONTEXT['apidata']['results']:
        paas_data = dict(paas_data)
        sored_keys = get_or_build_sshkey(paas_data, SSH_STORE_FOLDER)
        keyfile = safeget(sored_keys, 'credentials', 'ssh_key')
        uuname = keyfile[len(SSH_STORE_FOLDER) + 1:]
        paas_data["uuname"] = uuname
        paas_data["vm"] = []
        for vm in safeget(paas_data, 'related', 'virtualmachine', default=tuple()):
...
@login_required
def paas_wizardbox_create(request):
    ...
    CONTEXT.update( dict( api = dict(
        ...
        storages        = get_apidata('provisioning/iaac/storage/aws/list', request),   # hardcoded AWS before setting default storage - TODO
                                ) 
                        )
                    )
...
# 'provisioning/iaac/storage/<slug:provider>/list/'
# http://127.0.0.1:8001/api/v1/provisioning/iaac/image_groups/list/

def paas_create(request):
...
def paas_detail(request):
   
    CONTEXT.update( dict(
        api = get_apidata('usercredentials', request)
    ))

    return render(request, 'frontend/includes/%s/detail.html' % API_NAME, CONTEXT)


def paas_platforms(request):

    return render(request, 'frontend/includes/%s/platforms.html' % API_NAME, CONTEXT)
